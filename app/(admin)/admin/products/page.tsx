...
function PageContent() {
  ...
  const products =
    dataAny?.products?.docs ||
    dataAny?.data?.products?.docs ||
    dataAny?.products ||
    dataAny?.data?.products ||
    dataAny?.items ||
    dataAny?.results ||
    []
  const totalPages =
    dataAny?.pages ||
    dataAny?.totalPages ||
    dataAny?.products?.totalPages ||
    dataAny?.data?.pages ||
    dataAny?.data?.totalPages ||
    dataAny?.data?.products?.totalPages ||
    1
            ...
            return (
              <div key={p._id} className={`rounded-2xl border border-gray-100 p-4 shadow-sm ${p.isDeleted ? 'opacity-50' : ''}`}>
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-right text-sm font-bold text-gray-900">NGN {(p.price || 0).toLocaleString()}</div>
                </div>
...
export { PageContent as AdminProducts };
