...
const formatShortNumber = (value: any) => {
  const num = Number(value || 0);
...
const normalizeStatus = (value: any, fallback = "unknown") => {
  const raw = String(value || "").trim().toLowerCase();
  return raw || fallback;
};

const makeDateKey = (date: any) => new Date(date).toISOString().slice(0, 10);
...
export default function AdminAnalytics() {
  ...
  const trendData = useMemo(() => {
    ...
    orders.forEach((order) => {
      ...
      const entry = base.get(key) as any;
      entry.orders += 1;
      const payment = normalizeStatus(order?.paymentStatus);
  ...
  const categoryData = useMemo(() => {
    ...
    orders.forEach((order) => {
      ...
      items.forEach((item: any) => {
        ...
        const categoryLabel =
          product?.category?.name || product?.category || item?.category || item?.clothingType || "Uncategorized";
      ...
      <div className="rounded-3xl border border-gray-100 bg-gradient-to-br from-white via-gray-50 to-gray-100 p-6">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            <div key={card.label} className="rounded-2xl border border-gray-100 bg-white/80 p-4 shadow-sm">
          ...
          <div className="mt-4 h-64">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trendData}>
                ...
                <Tooltip formatter={(value, name) => (name === "revenue" ? formatCurrency(value) : value)} labelStyle={{ fontSize: 10 }} />
