...
function PageContent() {
  ...
  const paystackPublicKey = process.env.NEXT_PUBLIC_PAYSTACK_PUBLICKEY
  const useInlinePaystack = process.env.NEXT_PUBLIC_PAYSTACK_INLINE === 'true'
  const allowHostedPaystack = process.env.NEXT_PUBLIC_PAYSTACK_HOSTED === 'true'
  ...
  const pay = async () => {
    ...
    try {
      ...
      // 3. Paystack checkout (inline preferred, hosted fallback)
      const inlineCheckout = async () => {
        if (!paystackPublicKey || !user?.email) return false
        ...
        try {
          ...
          const handler = (window as any).PaystackPop.setup({
            ...
            callback: () => {
              ...
              window.location.href = `/payment/success?reference=${payData.reference}`
            },
            onClose: () => {
      ...
      let inlineOk = false
      if (useInlinePaystack) {
        inlineOk = await inlineCheckout()
        if (!inlineOk) {
          toast({
            title: 'Inline checkout unavailable',
            description: allowHostedPaystack
              ? 'Falling back to hosted checkout.'
              : 'Please enable hosted checkout or set your Paystack public key.',
            variant: allowHostedPaystack ? 'default' : 'destructive'
          })
        }
      }

      if (!useInlinePaystack || (!inlineOk && allowHostedPaystack)) {
        redirectingRef.current = true
        setTimeout(() => {
