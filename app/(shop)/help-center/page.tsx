...
const quickArticles = [
  { slug: "track-order", title: "How to Track Your Order" },
  { slug: "refunds", title: "Refunds and Chargebacks" },
  { slug: "account-security", title: "Account Security Basics" },
]
    ...
    <section className="bg-slate-50 px-4 py-10 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-4xl space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
          <h2 className="text-base font-bold text-slate-900">Quick Guides</h2>
          <p className="mt-1 text-sm text-slate-600">Deep dives on common questions.</p>
          <ul className="mt-3 space-y-2 text-sm font-semibold text-slate-800">
            {quickArticles.map((article) => (
              <li key={article.slug}>
                <Link href={`/help-center/${article.slug}`} className="hover:underline">
                  {article.title}
                </Link>
              </li>
            ))}
