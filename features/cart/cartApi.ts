...
const getVariantKey = (variant: ServerCartItem["variant"]) => {
...
const isHexLike = (value: unknown) =>
  /^[0-9a-fA-F]{3}([0-9a-fA-F]{3})?$/.test(String(value || ""));
...
const familyFromHex = (hex: unknown) => {
...
const sanitizeColorName = (name: unknown) => {
  if (!name) return ""
  return String(name).replace(/\s+[0-9a-fA-F]{3,6}$/, "").trim()
}

const isSizeLike = (value: unknown) => /^(size|spec|weight|size\/weight)\b/i.test(String(value || ""))

const getColorName = (rawColor: unknown) => {
  if (!rawColor) return "";
  ...
  const name = (rawColor as any).name || "";
  const hex = normalizeHex((rawColor as any).hex || "") || (isHexLike(name) ? normalizeHex(name) : "");
...
const buildVariantLabel = ({ variantSku, size, colorName }: { variantSku?: string | null; size?: string | null; colorName?: string | null; }) => {
...
export const normalizeCart = (cart: ServerCartItem[]): NormalizedCartItem[] => {
...
export const getCart = async () => {
  ...
  return normalizeCart(res.data.cart as ServerCartItem[]);
};

export const addToCart = async (productId: string, qty = 1, variant: ProductVariant | string | null = null) => {
...
export const updateCartItem = async (productId: string, qty: number, variant: ProductVariant | string | null = null) => {
...
export const removeCartItem = async (productId: string, variant: ProductVariant | string | null = null) => {
...
export const syncCart = async (items: NormalizedCartItem[]) => {
  const res = await axios.post(
    `${API}/sync`,
    { items },
    { withCredentials: true }
